...
4. Quality of Map - Due to the fact that the data that was readily available was categorised by Census Tract, the resulting map has an overly large number of units that make simple visual distinction difficult at low resolutions. Solutions include printing the image in large paper sizes or alternatively using an interactive map, as attempted in Part II.
...
To Export Simple Features File (CensusTractDataMap) - for use in other software such as ArcGIS/QGIS:
```{r}
